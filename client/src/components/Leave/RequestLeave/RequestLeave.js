...
import { leaveRequest, checkIfExists } from '../../../api/leave';
...
const RequestLeave = (props) => {
    ...
    const requestLeave = (event) => {
        ...
        checkIfExists(leaveData, sendRequest);
        ...
        <div>
            ...
            <Form onSubmit={(event) => requestLeave(event)} style={{ marginTop: "50px", marginBottom: "20px" }}>
                ...
                <div style = {{textAlign: "center"}}>
                    <span style={{ padding: "8px", fontSize: "1.1rem", fontWeight: "500" }}>Number of Days</span>
                    <input style={{ margin: "20px 47%", width: "6%" }} type="number" min="1" max="10" value={leaveDays} onChange={(event) => setLeaveDays(event.target.value)} />
                </div>
